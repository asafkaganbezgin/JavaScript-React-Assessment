...
// Initializing a temp object to be filled
let obj = {};
// Temp orgId variable.
let orgId;

positions.forEach((element) => {
    // Put name field from positions array into the temp object without any controls.
    obj.name = element.name;
    orgId = element.orgId;

    // Using the orgId relationship to find the organization name for the corresponding position name.
    organizations.forEach((element) => {
        if (element.id === orgId) {
            obj.orgName = element.name;
        }
    });

    //Add the temp object to the results array.
    positionsWithOrgNames.push(obj);

    // Flush the temp object.
    obj = {};
});
